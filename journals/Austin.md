<!-- 5/15/24
I spent an hour or so researching potential reasons for this pesky new 503 error I'm incurring. I found another major missing puzzle piece, when I input glv-cloud-cli list into my terminal to check on the deployed apps, neither of them were appearing - only "null". Considering I finished that last bit of CSS, I'm going to add, commit etc. to see if my pipelines pass or not because that could tell me a lot more about what's actually happening. All of the pipelines are passing & I see the CSS updates made in the browser now which is awesome but the teams still aren't being fetched from RapidAPI & the 503 error persists. Along with it showing not being deployed in the terminal. I believe I found the right command to deploy the project again via e-mail but I'm getting this in my terminal when I input the command,

{"error":"Sorry, we could not create the application... 🚧\n\nTroubleshooting Steps 🛠️:\n   ❌ Something may have happened on our end.\n   ✅ Report this issue if you believe there is a problem we\n      need to resolve.\n\n   ❌ Have you been assigned to a team?\n   ✅ Run the list command and check your team name is present.\n\n   ❌ If you have persistent volumes or environment variables,\n      they could be malformed.\n   ✅ Ensure that persistent volumes are linux style paths and\n      environment variables are formatted correctly. Run the\n      command 'glv-cloud-cli help deploy' for reference.\n\nGet Help 🛟:\n   Select «Platform Issue Report» at:\n   https://support.galvanize.com/hc/en-us/requests/new"}

If I can't deploy the project, then maybe this really is an error from Galvanize or RapidAPI? So I'll send them both messages to try & get a better idea as to what's happening because this appears to be something I've never dealt with before. However, I'm going to do some more troubleshooting before I reach out for help.
-->
